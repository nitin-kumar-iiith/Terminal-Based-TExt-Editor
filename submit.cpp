...
void init() {
    ...
    reset_filename();
    reset_statusmsg();
    //reset_statusmsg_time();
    if (getWindowSize(&E.screenrows, &E.screencols) == -1) die("getWindowSize");
    adjust_screenrows();//one for status bar and one for message bar
}
/*** Window Resize ***/
...
/*string fix_Destination(string source, string){
...
int getWindowSize(int *rows, int *cols){
    ...
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0){
        //if ioctl fails or returns col size=0, which can happen quite frequently
        //so we add a fallback method to get windows size even if it fails
        if (write(STDOUT_FILENO, "\x1b[999C\x1b[999B", 12) != 12){
            //even the fallback method failed
            die("getWindowSize: fallBack & ioctl Failed");
            return -1;
        } 
        else{
            //fallback method worked but we want to get cursor location
            unsigned int i = 0;
            char buf[32];
            ...
            while (i < sizeof(buf) - 1) {
                // iterate until you find 'R' and store keep storing in buffer 
            ...
            //now read from buffer the cursor location
            buf[i] = '\0';
            if (buf[0] != '\x1b' || buf[1] != '['){
            ...
            if (sscanf(&buf[2], "%d;%d", rows, cols) != 2){
                ...
                return -1;
                //storing row col value from buffer failed
            }
            //rows & cols value are updated so return 0 now
            return 0;
        }
    } 
    else{
        //ioctl didn't failed so we will get the contents from ws to rows and cols
        *cols = ws.ws_col;
        *rows = ws.ws_row;
        //rows & cols value are updated so return 0 now
        return 0;
...
int readKey() {
    ...
    while (true) {
        ...
        if (status == -1 && errno != EAGAIN) die("read");
        if(c == 'q'){//add crear scrren and exit
...
void move_cursor(int key) {////////////////////////////////////////////
    ...
    //erow *row = (E.g_cy() >= E.g_numrows()) ? NULL : &E.row[E.g_cy()];
    if(key == -10){    
            if (E.g_cy() != 0) {
                //E.cy--;
                int a=E.g_cy();
    ...
    //row = (E.g_cy() >= E.g_numrows()) ? NULL : &E.row[E.g_cy()];
    int rowlen;
    if(row){
        rowlen=row->size;
    }
...
void processKeyPress() {////////////////////////////////////////////
    ...
    if(c==-19){
        ...
        if (E.g_cy() < E.g_numrows())//go till last row of  that line
            E.s_cx(E.row[E.g_cy()].size);
    }
    else if((c==-7) ||(c==-8)){    
        if (c == -7){
...
void draw_rows(struct abuf *ab) {/////////////////////////////////////////////
    ...
    for (y = 0; y < screen_size; y++) {//print till dcreen size only,itterate over all row of screen
        ...
        if (y + E.g_rowoff() >= E.g_numrows()) {//number of rows to be is more than screen row size
            ...
            /*int len;// = E.row[y + E.g_rowoff()].rsize - E.g_coloff();
            if (E.row[y + E.g_rowoff()].rsize - E.g_coloff() < 0){
                len = 0;
            }
            if (E.row[y + E.g_rowoff()].rsize - E.g_coloff() > E.g_screencols()){
                len = E.g_screencols();
            }
            abAppend(ab, &E.row[y + E.g_rowoff()].render[E.g_coloff()], len);*/
...
void appendRow(char *s, size_t len) {///////////////////////////////////////
    ...
    E.row[E.g_numrows()].chars = (char*)malloc(len + 1);//check it can be (erow*) also
